...
@Named
@Scope("request")
public class RegisterIdaMedicalCaseBatchController {

	private static transient Logger LOGGER = Logger
			.getLogger(RegisterIdaMedicalCaseBatchController.class);
	...
	public String saveBatchEntry() {
		...
		try {
			...
			if (!getIsMobileCase()) {
				Collection<String> tokenNumberSet = new HashSet<String>();
				for (BatchIdaDog batchIdaDog : batchDogs) {
					tokenNumberSet.add(batchIdaDog.getTokenNo().toLowerCase());
				}
				if (!tokenNumberSet.isEmpty()) {
					if (batchDogs.size() != tokenNumberSet.size()) {
						facesUtil
								.addErrorMessage("Please select unique token number for each dog.");
						return null;
					}
					String tokenNumberError = controllerUtil
							.tokenNumberExixts(tokenNumberSet);
			...
			if (!isNoneCase) {
				...
				ngoCatcher = this.genericDao.getReference(NgoCatcher.class,
						Long.valueOf(ngoCatcherValue));
				ngoVehicle = this.genericDao.getReference(NgoVehicle.class,
						Long.valueOf(ngoVehicleValue));
			}
			...
			if (!IdaCasePersonType.NGO.name().equalsIgnoreCase(
				...
				this.idaCasePerson.setPersonType(IdaCasePersonType
						.getIdaCasePersonType(facesUtil.getFromSession(
								"personType").toString()));
			...
			for (BatchIdaDog batchIdaDog : batchDogs) {
				...
				if (breed == null) {
					throw new Exception(
							"Breed found as null !!!! for Street Dog");
				}
				...
				for (IdaCaseAdmissionReason idaCaseAdmissionReason : idaCaseAdmissionReasonList) {
					idaCaseBatch
							.addIdaCaseAdmissionReason(idaCaseAdmissionReason);
				}
				...
				idaCaseBatch.setSterlizationDate(batchIdaDog
						.getSterlizationDate());
			...
			ingestedBatchCase = this.genericDao.merge(getIngestedBatchCase());
